...
type MappingEnrolledCourseAdv struct {
	Year     string
	Semester string
	Courses  []EnrolledCourseAdv
}
...
func Scraping_Enrollcourse_adv(studentID string) ([]MappingEnrolledCourseAdv, error) {
	...
	var mappings []MappingEnrolledCourseAdv
	...
	var currentCourses []EnrolledCourseAdv
		...
		s.Find("table[cellspacing='1'][cellpadding='3'][width='60%'][border='0'][class='t'] tr[bgcolor='#FFFFFF']").Each(func(j int, row *goquery.Selection) {
			...
			currentCourses = append(currentCourses, EnrolledCourseAdv{
				CourseNo:   course_no,
		...
		if len(currentCourses) > 0 {
				mappings = append(mappings, MappingEnrolledCourseAdv{
					Year:     year,
					Semester: semester,
...
func GetEnrolledCoursesOldVersion(studentID string) ([]enrolledCourse, error) {
	body, err := read_http(studentID)
	if err != nil {
		logs.Error(err)
		return nil, errors.NewNotFoundError("Student ID not found")
		...
		s.Find("td[align='center'] > B").Each(func(j int, row *goquery.Selection) {
			...
			if strings.Contains(row.Text(), "ภาคเรียนที่") && strings.Contains(row.Text(), "ปีการศึกษา") {
				...
				if len(semesterMatches) >= 2 {
					...
					studentID_transformed, err := transformInput(studentID)
					num_studentID, nil := strconv.Atoi(studentID_transformed)
					if err != nil {
...
func GetEnrolledCourseByStudentIDOldVersion(id string) map[string]map[string][]enrolledCourse {
	// Input your student ID here
	studentID := id

	courses, err := GetEnrolledCoursesOldVersion(studentID)
	if err != nil {
		log.Fatal(err)
	}
